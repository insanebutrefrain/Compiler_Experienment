...
public:
    // 构造函数
    LR(string expressionPath = "P:\\Clion\\Compiler\\exp5_LR\\file\\expression.txt") : lexer(expressionPath) {
    }

    string expressionPath = "P:\\Clion\\Compiler\\exp5_LR\\file\\expression.txt";
    string processPath = "P:\\Clion\\Compiler\\exp5_LR\\file\\process.csv";
    string resultPath = "P:\\Clion\\Compiler\\exp5_LR\\file\\result.txt";
    string grammarPath = "P:\\Clion\\Compiler\\exp5_LR\\file\\g.txt";
 ...
 */
    ...
    // 测试单个表达式
    bool testSingleExpression(string expression, ofstream &processStream) {
        int step = 0;
        stack<int> stateStack({0});
        stack<char> symbolStack({'#'}), expressionStack({'#'});
        ...
        while (!stateStack.empty() && !expressionStack.empty() && !symbolStack.empty()) {
            string comment = " ";
            Action action = actionTable[stateStack.top()][expressionStack.top()];
            if (action.act == ERROR) return false;
            else if (action.act == ACC) return true;
            else if (action.act == SHIFT) { //移进
                stateStack.push(action.content);
                symbolStack.push(expressionStack.top());
                expressionStack.pop();
    ...
    // 记录符号栈的变化
    void logPrecess(ofstream &processStream, int step, stack<int> stateStack, stack<char> symbolStack,
        ...
        while (!expressionStack.empty()) {
            allExpression += string(1, expressionStack.top()) + " ";
            expressionStack.pop();
        }
        reverse(allSymbol.begin(), allSymbol.end());
        processStream << step << "," << allState << "," << allSymbol << "," << allExpression << "," << comment
                      << endl;
    }
};